...
public interface TemaDAOQuery {
    ...
    public final static String GET_TEMAS_FROM_GRUPO = "select hex(id) as id, hex(userid) as userid, hex(grupoid) as grupoid, titulo, contenido, creation_timestamp from temas WHERE grupoid=unhex(?) order by creation_timestamp desc ";
