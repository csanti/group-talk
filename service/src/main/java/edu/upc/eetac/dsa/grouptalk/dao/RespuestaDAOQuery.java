...
public interface RespuestaDAOQuery {
    ...
    public final static String GET_RESPUESTAS_FROM_TEMA = "select hex(id) as id, hex(userid) as userid, hex(temaid) as temaid, contenido, creation_timestamp from respuestas where temaid=unhex(?) order by creation_timestamp desc";
